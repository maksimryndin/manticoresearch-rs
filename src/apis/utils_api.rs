...
pub async fn sql(configuration: &configuration::Configuration, body: &str, raw_response: Option<bool>) -> Result<Vec<serde_json::Value>, Error<SqlError>> {
    ...
    if let Some(ref local_var_str) = raw_response {
        local_var_req_builder = local_var_req_builder.query(&[("raw_response", &local_var_str.to_string())]);
    }
